...
> **1.0 决议（2026-09-07）**：新增**侵入模式**作为显式 opt-in 的受控例外。默认模式仍完全
> 遵守本节原则；侵入模式在用户通过警告确认弹窗明确同意后，仅补丁原生 `MathJax` 对象的
> `tex2chtml` 与 `chtmlStylesheet` 两个成员（不删除、不替换全局对象，`tex2svg` 等其他成员
> 不动），带运行时特性守卫、守卫失败自动回退默认模式、禁用/卸载时逐字节还原并在恢复后把
> 已渲染公式交还原生渲染器。该例外限定于 `src/invasive/`，契约记录在 README 与
> `docs/compatibility.md`；"静默、无条件或更广范围的全局改动"仍然禁止。
